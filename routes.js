...
routes.get('/', (req, res)=>{
    req.getConnection((err, conn) => {
        ...
        conn.query('SELECT * FROM prestamos', (err, rows) => {
            if(err) return res.send(err)
...
routes.post('/', (req, res)=>{
    req.getConnection((err, conn) => {
        if(err) return res.send(err)        
       conn.query('INSERT INTO prestamos set ?', [req.body], (err, rows) => {
            if(err) return res.send(err)
    ...
    routes.delete('/:id', (req, res)=>{
        req.getConnection((err, conn) => {
            if(err) return res.send(err)        
           conn.query('DELETE FROM prestamos WHERE id = ?', [req.params.id], (err, rows) => {
                if(err) return res.send(err)
